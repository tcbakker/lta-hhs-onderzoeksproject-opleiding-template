...
-   Voor **databewerking en statistische** **analyses** is [R](https://www.r-project.org/) leidend met als editor [R Studio](https://posit.co/download/rstudio-desktop/) van Posit; bij uitzondering kan ook Python gebruikt worden. Iedere analyse wordt als project opgesteld.
    -   Maak gebruik van de huisstijlgids die het lectoraat hanteert (opvragen bij de lector).
    -   Gebruik de LTA-snippets om efficiënt te kunnen coderen (opvragen bij de lector).
...
-   **Broncode** leggen we vast en delen we via [Git](https://github.com/) (Github). Broncode is -- als het wordt ontwikkeld binnen je aanstelling voor het lectoraat -- eigendom van De HHs en daarbinnen het lectoraat.
...
3.  **Maak de uitgebreide documentatie**. Dit wordt uitgevoerd door het `dataMaid` package. De uikomsten worden automatisch opgeslagen in de folder 'Documentatie/Documentatie_LTA_uitgebreid'; het rapport opent zich in je browser. Zie [Figuur 2](#dataMaid).
